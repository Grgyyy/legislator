...
class TrainingProgramExport implements FromQuery, WithMapping, WithStyles, WithHeadings, WithDrawings, WithColumnWidths
{
    ...
    {
        return [
            $this->getQualificationCode($record),
            $record->soc_code ?? '-',
            $record->full_coc_ele ?? '-',
            $record->nc_level ?? '-',
            $record->title ?? '-',
            $record->scholarshipPrograms->pluck('name')->implode(', '),
            $record->tvet->name ?? '-',
    ...
    private function getQualificationCode($record)
    {
        return empty($record->nc_level) ? '-' : $record->code;
    }
